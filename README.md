...
This section will cover the steps needed to run our validation procedure to determine the ability of CoralNet's automatic annotations to estimate percent cover of key substrates at the scale of a transect segment.  The purpose of this is to determine how well the CoralNet algorithm estimates percent cover of substrates compared to a unique reference set.  This reference set is created using 12 manually-annoated photographs to estimate the percent cover at the scale of a transect segment (roughly 10m of linear reef area).  The percent cover from different numbers of unique manually-annotated and CoralNet-annotated photographs are then calculated, creating the sample set.  For each transect segment, a regression is calculated using the percent cover from the sample set as the predictor and the percent cover from the reference set as the response.  The process is repeated thousands of times with different photographs used for each sample 
