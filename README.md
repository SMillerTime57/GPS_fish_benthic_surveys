...
Contains code and documentation to process and validate data collected using the GPS-enabled, paired fish and benthic surveys described in ______.  It is assumed that the reader has read this paper and supplement because some terminology is used that is not repeated here.  Examples of metadata files are included to show the proper format for the associated functions to work.  Data and code for the fish and benthic validation procedures are included.  The script to run the fish validation is "method_comparison.R", and the script to run the validation for benthic surveys is "sample_validation.R".
...
- set FIILELIST= should be the pathway to a .txt file containing just a list of photograph names without a header or the file extension included (the output from the "extracted_images" file produced by the "photo_extractor.R" script provides the proper file names but will need to be saved as a .txt file and the default header removed) -- ex., set FIILELIST=D:\example_folder\extracted_images.txt
...
For our validation procedure, all photographs from random transect segments (we used 50 transect segments) need to be annotated; 21 are done manually and the other 69 are automatically annotated by CoralNet.  These photograph associated with individual transect segments can be identified from the metadata generated by timeLapseTimer.R.  When selecting the 21 photographs to be manually annotated, we used images evenly spaced throughout the transect to ensure proper spatial coverage of the reference images.  Once these are calculated, the dataframes need to be converted to "wide" format, with columns for the percent covers of the different substrates and additional columns for an identifier for the photographs ("Name") and one called "SurveyCode" that is an identifier for the transect segment.  Each row represents a unique photograph.
...
Included in this repository is an R script, "sample_validation.R" and accompanying data that shows how these can be used to replicate the results from the accompanying paper and can be used as a starting point for your own validations.  Note that running "sample_validation.R" as-is will replicate results from the accompanying paper, but will take a very long time.  If you just desire to play with the code and get a feel for how the functions operate, you may want to consider reducing the number of replicates.  The behavior of the functions is described in detail below:
